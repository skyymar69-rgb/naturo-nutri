...
export function generateMetadata({ params }: PageProps): Metadata {
  ...
  return {
    ...
    description: `Profil ${t.nom.toLowerCase()} : alimentation, nutrithérapie (Curtay), jeûnes (Grosjean), purges et plantes adaptés à ce tempérament hippocratique.`,
  };
}
  ...
  return (
    <>
      ...
      <Container className="py-12 space-y-16">
        ...
        <section>
          <SectionTitle icon={Leaf} title="Alimentation" source="grosjean" />
          <div className="grid lg:grid-cols-2 gap-6">
            <Card accent="forest">
              <CardContent>
                <CardTitle as="h3" className="text-lg mb-4 text-sage-700">
                  ✓ À privilégier
                </CardTitle>
            ...
            <Card accent="earth">
              <CardContent>
                <CardTitle as="h3" className="text-lg mb-4 text-earth-700">
                  ✗ À éviter ou limiter
                </CardTitle>
                <ul className="space-y-2 text-sm text-forest-800/90">
          ...
          <WarningBox variant="info" title="Note de l'école Grosjean" className="mt-6">
            <p>{t.alimentation.notes_grosjean}</p>
          </WarningBox>
        </section>

        <section>
          <SectionTitle icon={FlaskConical} title="Nutrithérapie" source="curtay" />
          ...
          {t.nutritherapie.a_eviter.length > 0 && (
            <WarningBox variant="warning" title="À éviter en complémentation" className="mt-6">
              <ul className="space-y-1">
                {t.nutritherapie.a_eviter.map((a) => (
                  <li key={a}>• {a}</li>
                ))}
              </ul>
            </WarningBox>
          ...
          <WarningBox variant="info" title="Note du Dr Curtay" className="mt-6">
            <p>{t.nutritherapie.notes_curtay}</p>
          </WarningBox>
        </section>

        <section>
          <SectionTitle icon={Droplets} title="Jeûnes & cures" source="grosjean" />
          <div className="grid sm:grid-cols-2 gap-4">
            {t.jeunes.map((j) => (
              <Card key={j.type} accent="sage">
                <CardContent className="p-5">
                  <h4 className="font-display text-lg text-forest-900 mb-1">{j.type}</h4>
                  <p className="text-xs uppercase tracking-wider text-forest-600 mb-3">
                    Durée · {j.duree}
                  </p>
                  <p className="text-sm text-forest-800/90 leading-relaxed mb-3">{j.note}</p>
                  <div className="flex flex-wrap gap-1.5">
                    {j.saison_ideale.map((s) => (
                      <Badge key={s} variant="sage" className="text-xs capitalize">
                        {s}
                      </Badge>
                    ))}
                  </div>
        ...
        <section>
          <SectionTitle icon={Droplets} title="Purges & drainages" source="grosjean" />
          <div className="space-y-4">
            {t.purges.map((p) => (
              <Card key={p.nom} accent="cream">
                <CardContent className="p-5">
                  <h4 className="font-display text-lg text-forest-900 mb-2">{p.nom}</h4>
                  <p className="text-sm text-forest-800/90 mb-2">
                    <strong>Protocole :</strong> {p.protocole}
                  </p>
                  <p className="text-xs text-forest-600">
                    <strong>Fréquence :</strong> {p.frequence}
                  </p>
                </CardContent>
              </Card>
            ))}
        ...
        <section>
          <SectionTitle icon={Sprout} title="Plantes alliées" source="tradition" />
        ...
        <section>
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {otherTemperaments.map((s) => {
              ...
              return (
                <Link key={s} href={`/temperaments/${s}`} className="group">
...
function Header({ temperament: t }: { temperament: typeof TEMPERAMENTS[TemperamentSlug] }) {
  return (
    <section
      className="relative py-16 sm:py-24 text-cream-50 overflow-hidden"
      style={{
        background: `linear-gradient(135deg, ${t.couleurHex}, ${t.couleurHex}99)`,
      }}
    >
      ...
      <Container className="relative">
        <Link href="/temperaments" className="text-sm text-cream-50/80 hover:text-cream-50 mb-4 inline-block">
        ...
        <div className="grid lg:grid-cols-3 gap-8 items-center">
          <div className="lg:col-span-2">
            ...
            <p className="mt-6 text-lg text-cream-50/95 leading-relaxed max-w-2xl">
              {t.description}
            </p>
...
function SectionTitle({
  ...
  icon: typeof Leaf;
  title: string;
  source: 'grosjean' | 'curtay' | 'tradition' | 'les_deux';
}) {
  return (
    <div className="mb-6 flex items-center justify-between gap-4 flex-wrap">
      <h2 className="font-display text-3xl sm:text-4xl text-forest-900 flex items-center gap-3">
