...
export function generateStaticParams() {
  return ALL_ARTICLES
    .filter((a) => a.domain === 'nutritherapie')
...
export function generateMetadata({ params }: PageProps): Metadata {
  ...
  const canonical = `${SITE}/nutritherapie/${params.category}/${params.slug}`;
  return {
    title: article.title,
...
export default function ArticlePage({ params }: PageProps) {
  ...
  const allInCat = getArticlesByCategory('nutritherapie', params.category);
  ...
  const url = `${SITE}/nutritherapie/${params.category}/${params.slug}`;
    ...
    <>
      ...
      <BreadcrumbJsonLd
        crumbs={[
          ...
          { name: 'Nutrithérapie', url: `${SITE}/nutritherapie` },
          { name: cat?.nom ?? params.category, url: `${SITE}/nutritherapie/${params.category}` },
      ...
      <div className="w-full bg-earth-50/30">
        <Container size="prose" className="pt-8 pb-0">
          <figure className="relative w-full overflow-hidden rounded-2xl shadow-lg ring-1 ring-earth-100">
            <ArticlePhoto src={heroImg.src} alt={heroImg.alt} ratio="21/9" priority />
            <figcaption className="absolute top-3 left-3">
              <span className="inline-flex items-center gap-1 bg-white/90 backdrop-blur-sm text-earth-800 text-xs font-semibold px-3 py-1.5 rounded-full shadow-sm">
        ...
        <div className="grid lg:grid-cols-[1fr_220px] gap-12">
          <article className="max-w-3xl mx-auto lg:mx-0 w-full space-y-10">
            <div className="prose-natural">
              <p className="text-lg leading-relaxed text-forest-800 border-l-4 border-earth-300 pl-5 py-1 bg-earth-50/50 rounded-r-xl drop-cap">
                {article.intro}
              </p>
            </div>
                  ...
                  <ul className="space-y-2">
                    {article.keyPoints.map((k) => (
                      <li key={k} className="flex gap-2.5 text-forest-800/90 text-sm">
                        <span className="text-earth-500 font-bold mt-0.5 flex-shrink-0" aria-hidden="true">▸</span>
                        <span>{k}</span>
                      </li>
                    ))}
                  </ul>
            ...
            {(prev || next) && (
              <nav aria-label="Article précédent et suivant" className="grid sm:grid-cols-2 gap-4 pt-2 border-t border-forest-100">
                {prev ? (
                  <Link
                    href={`/nutritherapie/${prev.category}/${prev.slug}`}
                    className="group bg-cream-50 hover:bg-earth-50 border border-forest-100 rounded-2xl p-4 transition-colors"
                  >
                    <p className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-earth-600 mb-1">
                ...
                {next ? (
                  <Link
                    href={`/nutritherapie/${next.category}/${next.slug}`}
                    className="group bg-cream-50 hover:bg-earth-50 border border-forest-100 rounded-2xl p-4 text-right transition-colors"
                  >
                    <p className="flex items-center justify-end gap-1.5 text-xs font-semibold uppercase tracking-wide text-earth-600 mb-1">
            ...
            <div className="text-center pt-4">
              <ButtonLink href={`/nutritherapie/${cat?.slug}`} variant="secondary">
