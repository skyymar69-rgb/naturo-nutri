...
import { getCategory, NATURO_CATEGORIES } from '@/lib/categories';
import { getArticlesByCategory } from '@/lib/articles';

export function generateStaticParams() {
  return NATURO_CATEGORIES.filter((c) => c.slug !== 'temperaments').map((c) => ({ category: c.slug }));
}
...
export default function CategoryPage({ params }: PageProps) {
  ...
  const articles = getArticlesByCategory('naturopathie', params.category);

  return (
    <>
      <section className="grain-bg py-16 sm:py-20">
        <Container>
          {/* Breadcrumb / back link — aria compliant */}
          ...
          <div className="flex items-start gap-6">
            {/* Emoji décoratif — aria-hidden car le h1 nomme la catégorie */}
            ...
            <div>
              ...
              {/* text-forest-900 sur grain-bg (cream-50 clair) → 10.6:1 ✅ */}
              <h1 className="font-display text-5xl sm:text-6xl text-forest-900 leading-tight">
                {cat.nom}
              </h1>
              {/* text-forest-700 solid sur cream-50 → 7.25:1 ✅ */}
          ...
          {articles.length === 0 ? (
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {articles.map((art) => (
                <Link
                  key={art.slug}
                  href={`/naturopathie/${cat.slug}/${art.slug}`}
                ...
                >
                  <Card hoverable accent="forest" className="h-full">
                    <CardContent>
                      ...
                      {/* text-forest-700 solid → 7.25:1 ✅ (no opacity) */}
